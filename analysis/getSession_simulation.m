...
function session = getSession_simulation(features, spikes, padding_opotogenetic_pulse, varargin)

    p = inputParser;
    addRequired(p,'features');
    addRequired(p,'spikes',@isstruct);
    addRequired(p,'padding_opotogenetic_pulse',@isnumeric);
    ...
    parse(p,features,spikes,padding_opotogenetic_pulse,varargin{:});
    ...
    if exist([basepath filesep sessionName '.session.mat'],'file') && forceReload == false
        disp('Loading file...');
        load([basepath filesep sessionName '.session']);
    else
        ...
        if saveMat
            save([basepath filesep sessionName '.session.mat'],'session');
        end
    end
